...
{ 
  private:
    ...
    pcap_t *handle;
    unordered_map<uint64_t,u_int> flow_table;
  ...
  public:
    ...
    PCAP_IP_tap(const char* iface, int snaplen,
        ...
        this->flow_table = unordered_map<uint64_t,u_int>();
        if(handle==NULL){
    ...
    void packet_handler(const struct pcap_pkthdr *metadata,const u_char *data){
            ...
            uint64_t tmp = 0 | (ih->saddr);
            uint64_t key = (tmp << 32) | (ih->daddr);
            this->flow_table[key]++;

            //cout << this->flow_table[key] << endl;
...
{
    char ip_src[16], ip_dst[16];
    for(auto elem : pit.flow_table){
        int saddr, daddr;
        saddr = 0 | (elem.first >> 32);
        daddr = 0 | (elem.first);
        //src addr
        inet_ntop(
            AF_INET,
            reinterpret_cast<const void *>(&saddr),
            ip_src,
            sizeof(ip_src)
        );
        ...
        inet_ntop(
            AF_INET,
            reinterpret_cast<const void *>(&daddr),
            ip_dst,
            sizeof(ip_dst)
        );

        os << "src: " << ip_src << ", dst: " << ip_dst << ": " << elem.second << endl;
